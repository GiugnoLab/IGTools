...
public class DistalDistributionsFrame extends javax.swing.JFrame {
    ...
    private void makeCharts(){
        try{
           ...
           Distribution est = distrExtractor.estimateGeometric(igtools.analyses.recurrences.carpena.DistalRecurrence.proper_codistances(wordIt));
    ...
    private ChartPanel makeRawDistalDistrChart(Double maxDist){
        try{
            int[] dists = igtools.analyses.recurrences.carpena.DistalRecurrence.proper_codistances(wordIt);
            final XYSeries series = new XYSeries("");
            if(dists != null){
                if(dists.length > 0){
    ...
    private ChartPanel makeEstGeomDistrPanel(IELSAIterator it, double maxDist){
        try{
            ...
            int[] dists = igtools.analyses.recurrences.carpena.DistalRecurrence.proper_codistances(it);
            double ep = 0.0;
            if(dists != null){
                if(dists.length > 0){
                    double n = 0.0;
