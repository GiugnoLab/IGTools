...
//            }
        ...
        */
        ...
        if(it_1.compare(it_2) == 0){
            if(!overlapping){
                dists = igtools.analyses.recurrences.carpena.DistalRecurrence.proper_codistances(it_1);
            }
            else{
                dists = igtools.analyses.recurrences.carpena.DistalRecurrence.proper_overlapping_codistances(it_1);
                System.out.println(dists);
            }
        }
        else{
            if(!overlapping){
                dists = igtools.analyses.recurrences.carpena.DistalRecurrence.codistances(it_1, it_2);
            }
            else{
                dists = igtools.analyses.recurrences.carpena.DistalRecurrence.overlapping_codistances(it_1, it_2);
            }
        }
        ...
        if(dists != null){
            if(dists.length > 0){
                int[][] codists = igtools.analyses.recurrences.carpena.DistalRecurrence.co_recurrences_array(dists, false);
                for(int i=0; i<codists[0].length; i++){
                    if(codists[0][i] > maxDist)
                        break;
                    series.add(codists[0][i], codists[1][i]);
                    System.out.println(codists[0][i]+" "+ codists[1][i]);
                }
            }
        }
